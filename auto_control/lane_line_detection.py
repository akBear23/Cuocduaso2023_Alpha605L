...
def find_left_right_points(image, draw=None):
    ...
    for x in range(center + 1, im_width):
        if interested_line[x] > 0:
            ...
            break
    flag = 0
    if(left_point == -1 and right_point == -1): flag = 1
    ...
    return left_point, right_point, flag


def calculate_control_signal(img, sign=None, draw=None):
    """Calculate speed and steering angle
    """

    # direct = '/home/ankhanh/CuocDuaSo/data/fetch/pic.jpg'
    # cv2.imwrite(direct, draw)

    # Find left/right points
    img_lines = find_lane_lines(img)
    img_birdview = birdview_transform(img_lines)
    draw[:, :] = birdview_transform(draw)
    left_point, right_point, flag = find_left_right_points(img_birdview, draw=draw)
    ...
    if left_point != -1 and right_point != -1:
        ...
        if(sign == None):
            steering_pid = PID(0.02, 0.00, 0.1, setpoint=0)
            steering_angle = -steering_pid(-center_diff)
        if(sign == 'right'):
            print(sign)
            if flag:
                steering_angle += 0.8
...
def calculate_control_signal_segment(img):
    ls_cx, ls_cy = measureLane(img)
